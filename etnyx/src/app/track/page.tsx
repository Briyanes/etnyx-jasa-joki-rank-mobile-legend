...
const t = {
  id: {
    ...
    yourOrder: "Order",
    you: "Kamu",
    subtitle: "Masukkan Order ID untuk melihat progress joki kamu secara real-time",
    ...
    question: "Ada pertanyaan tentang order ini?",
    chatWA: "Chat via WhatsApp",
    helpText: "Order ID ada di chat WhatsApp konfirmasi order kamu",
    format: "Format: ETX-XXXXXXXX",
  ...
  en: {
    ...
    yourOrder: "Your",
    you: "Order",
    subtitle: "Enter Order ID to see your boosting progress in real-time",
    placeholder: "Example: ETX-ABC12345",
    trackBtn: "Track",
    searching: "Searching...",
    safe: "Safe",
    fast: "Fast",
    orderId: "Order ID",
    progress: "Progress",
    pushingAt: "Currently pushing at rank:",
    statusTimeline: "Status Timeline",
    username: "Username",
    package: "Package",
    ...
    question: "Have questions about this order?",
    chatWA: "Chat via WhatsApp",
    helpText: "Order ID is in your WhatsApp confirmation message",
    format: "Format: ETX-XXXXXXXX",
...
function TrackOrderContent() {
  ...
  const statusConfig: Record<string, { label: string; color: string; icon: ReactNode }> = {
    pending: { label: txt.status.pending, color: "text-yellow-400", icon: <Clock className="w-4 h-4" /> },
    confirmed: { label: txt.status.confirmed, color: "text-blue-400", icon: <Check className="w-4 h-4" /> },
    in_progress: { label: txt.status.in_progress, color: "text-accent", icon: <Rocket className="w-4 h-4" /> },
    completed: { label: txt.status.completed, color: "text-green-400", icon: <CheckCircle className="w-4 h-4" /> },
  ...
  const trackOrder = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || txt.orderNotFound);
        return;
      }

      setOrder(data);
    } catch {
      setError(txt.orderNotFound);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 sm:gap-4 text-xs text-text-muted">
            <span className="flex items-center gap-1.5 hidden sm:flex">
              <Shield className="w-3.5 h-3.5 text-success" /> {txt.safe}
            </span>
            <span className="flex items-center gap-1.5 hidden sm:flex">
              <Zap className="w-3.5 h-3.5 text-yellow-400" /> {txt.fast}
            </span>
            <span className="flex items-center gap-1.5 hidden sm:flex">
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="text-center mb-12">
          <h1 className="text-3xl md:text-4xl font-bold text-text mb-4">
            {txt.trackOrder} <span className="text-primary">{txt.yourOrder}</span> {txt.you}
          </h1>
        ...
        {loading && !order && (
          <div className="max-w-2xl mx-auto text-center py-12">
            ...
            <p className="text-muted">{txt.searching}</p>
          </div>
        )}
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="bg-surface rounded-2xl p-6 md:p-8 border border-surface/50 shadow-xl">
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8 pb-6 border-b border-surface/50">
                <div>
                  <p className="text-muted text-sm mb-1">{txt.orderId}</p>
              ...
              <div className="mb-8">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-muted text-sm">{txt.progress}</span>
                ...
                {order.current_progress_rank && (
                  <p className="text-sm text-muted mt-2">
                    {txt.pushingAt} <span className="text-accent font-medium">{rankLabels[order.current_progress_rank] || order.current_progress_rank}</span>
                  </p>
                )}
              </div>
              ...
              {/* Timeline */}
              <div className="mb-8">
                <p className="text-muted text-sm mb-4">{txt.statusTimeline}</p>
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-background rounded-xl">
                <div>
                  <p className="text-muted text-xs mb-1">{txt.username}</p>
        ...
        {!order && !loading && (
          <div className="text-center text-muted">
            <p className="mb-2">{txt.helpText}</p>
            <p className="text-sm">{txt.format}</p>
          </div>
        )}
      </div>
    </main>
