    ...
    <footer className="relative overflow-hidden">
      ...
      <div className="relative pt-10 sm:pt-14 pb-6 sm:pb-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          {/* WhatsApp CTA Box */}
          <div className="mb-8 sm:mb-12 rounded-2xl bg-gradient-to-r from-emerald-500/10 to-green-500/5 border border-emerald-500/20 p-5 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-emerald-500/20 border border-emerald-500/30 flex items-center justify-center flex-shrink-0">
              ...
              <div className="text-left">
                <p className="text-text font-bold text-sm sm:text-base">
                  {locale === "id" ? "Butuh Bantuan? Chat Kami!" : "Need Help? Chat with Us!"}
                </p>
                <p className="text-text-muted text-xs sm:text-sm">
                  {locale === "id" ? "Tim kami online 24/7 di WhatsApp" : "Our team is online 24/7 on WhatsApp"}
                </p>
              </div>
            </div>
            <a
              href={`https://wa.me/${socialLinks?.whatsapp || WHATSAPP_NUMBER}`}
              target="_blank"
              rel="noopener noreferrer"
              onClick={() => { import("@/lib/tracking").then(m => m.trackLead()); }}
              className="px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold transition-all duration-200 hover:scale-105 flex items-center gap-2 whitespace-nowrap"
            >
